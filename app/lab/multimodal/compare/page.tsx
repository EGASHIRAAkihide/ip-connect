...
import { runMultimodalCompare } from "./actions";

export default async function MultimodalComparePage() {
  await requireLabAdmin();

  return (
    <section className="mx-auto max-w-3xl space-y-6 py-8">
      <header className="space-y-2">
        <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Multimodal Compare</p>
        <h1 className="text-3xl font-semibold text-neutral-900">Audio × Choreo 比較 (A/B)</h1>
        <p className="text-sm text-neutral-700">
          A/Bの音声ピーク系列と動きピーク系列を簡易比較し、AI実験ログとして保存します。参考値です。
        </p>
      </header>

      <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
        <form action={runMultimodalCompare} className="space-y-4">
