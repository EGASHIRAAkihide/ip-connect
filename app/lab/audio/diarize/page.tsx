import { runDiarization } from "./actions";
import { requireLabAdmin } from "@/lib/lab";

export default async function LabDiarizePage() {
  await requireLabAdmin();

  return (
    <section className="mx-auto max-w-3xl space-y-6 py-8">
      <header className="space-y-2">
        <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Diarization</p>
        <h1 className="text-3xl font-semibold text-neutral-900">話者ダイアリゼーション</h1>
        <p className="text-sm text-neutral-700">
          音声を話者ごとに区間分割します。本人一致を保証しない参考情報です。
        </p>
      </header>

      <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
        <form action={runDiarization} className="space-y-4">
          <div>
