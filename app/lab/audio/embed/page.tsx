import { runEmbedding } from "./actions";
import { requireLabAdmin } from "@/lib/lab";

export default async function LabEmbedPage() {
  await requireLabAdmin();

  return (
    <section className="mx-auto max-w-3xl space-y-6 py-8">
      <header className="space-y-2">
        <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Speaker Embedding</p>
        <h1 className="text-3xl font-semibold text-neutral-900">話者埋め込み生成</h1>
        <p className="text-sm text-neutral-700">
          音声から話者埋め込みベクトルを生成します。本人一致を保証しない参考情報です。
        </p>
      </header>

      <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
        <form action={runEmbedding} className="space-y-4">
          <div>
