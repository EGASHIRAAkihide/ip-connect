...
export async function runChoreoPhraseCompare(formData: FormData) {
  ...
  const topKRaw = Number(formData.get("top_k") ?? 3);
  const sampleFpsRaw = Number(formData.get("sample_fps") ?? 10);
  const maxSecondsRaw = Number(formData.get("max_seconds") ?? 60);
  const topK = Number.isFinite(topKRaw) && topKRaw > 0 ? Math.floor(topKRaw) : 3;
