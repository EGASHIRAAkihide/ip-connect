import { runChoreoPhraseCompare } from "./actions";
import { requireChoreoLabAdmin } from "@/lib/lab";

export default async function ChoreoPhraseComparePage() {
  await requireChoreoLabAdmin();

  return (
    <section className="mx-auto max-w-3xl space-y-6 py-8">
      <header className="space-y-2">
        <p className="text-xs uppercase tracking-[0.2em] text-neutral-500">Choreo Phrase Compare</p>
        <h1 className="text-3xl font-semibold text-neutral-900">フレーズ単位の類似度マッチング</h1>
        <p className="text-sm text-neutral-700">
          動画Aの各フレーズに対して、動画BのTop-K候補を骨格ランドマーク埋め込みでマッチングします。参考値です。
        </p>
      </header>

      <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
        <form action={runChoreoPhraseCompare} className="space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <label className="flex flex-col text-xs font-semibold uppercase tracking-wide text-neutral-600">
              ...
              <input
                ...
                defaultValue={3}
              />
              <span className="mt-1 text-[11px] text-neutral-500">Aフレーズごとに返すTop-K候補数</span>
            </label>
            <label className="flex flex-col text-xs font-semibold uppercase tracking-wide text-neutral-600">
              sample_fps
              <input
