...
export function ChoreoPhraseReview({ videoA, videoB, matches = [], phrases = [], signError }: Props) {
          ...
          <table className="min-w-full divide-y divide-neutral-200 text-sm">
            <thead className="bg-neutral-50 text-left text-xs uppercase tracking-wide text-neutral-600">
              <tr>
                ...
                <th className="px-3 py-2">Key Joints</th>
            ...
            <tbody className="divide-y divide-neutral-200">
              {phrases.map((p, idx) => (
                <tr key={`${p.phrase_id ?? "p"}-${idx}`} className="align-top hover:bg-neutral-50">
                  ...
                  <td className="px-3 py-2 text-xs text-neutral-700">
                    {(p.key_joints ?? []).length > 0 ? (p.key_joints ?? []).join(", ") : "—"}
                  </td>
                  <td className="px-3 py-2 text-xs text-neutral-700">{p.confidence ?? "—"}</td>
                  <td className="px-3 py-2 text-right">
                    <button
                      type="button"
                      onClick={() => jumpBoth(p.start)}
